...
def getAvgBuyOrders(output, security):
    ...
    if 'buy' in output and output['symbol'] == security:
        ...
        maximum = max(orders)
        print(str(avg))
    return {'min': minimum, 'max': maximum, 'avg': avg, 'volume': volume}
...
def main():
    ...
    while True:
        ...
        # write_to_exchange(exchange, {"type": "add", "order_id": int(timeid) + random.randint(1,1001), "symbol": "BOND", "dir": "BUY", "price": 998, "size": 30})
        # write_to_exchange(exchange, {"type": "add", "order_id": int(timeid) + random.randint(1,1001), "symbol": "BOND", "dir": "SELL", "price": 1002, "size": 30})
        ...
        adr_buy = getAvgSellOrders(message, "BABA")
        etf_buy = getAvgSellOrders(message, "BABZ")

        adr_buy_avg = adr_buy['max']
        etf_buy_avg = etf_buy['max']

        if adr_buy_avg < etf_buy_avg:
            write_to_exchange(exchange,
                              {"type": "add", "order_id": int(timeid) + random.randint(1, 1001), "symbol": "BABA",
                               "dir": "BUY", "price": int(adr_buy['max'] - 3), "size": adr_buy['volume'] / 3})
            write_to_exchange(exchange,
                              {"type": "add", "order_id": int(timeid) + random.randint(1, 1001), "symbol": "BABZ",
                               "dir": "SELL", "price": int(etf_buy['max'] + 3), "size": adr_buy['volume'] / 4})
        elif adr_buy_avg > etf_buy_avg:
            write_to_exchange(exchange,
                              {"type": "add", "order_id": int(timeid) + random.randint(1, 1001), "symbol": "BABA",
                               "dir": "SELL", "price": int(adr_buy['min'] + 3), "size": adr_buy['volume'] / 3})
            write_to_exchange(exchange,
                              {"type": "add", "order_id": int(timeid) + random.randint(1, 1001), "symbol": "BABZ",
                               "dir": "BUY", "price": int(etf_buy['min'] - 3), "size": adr_buy['volume'] / 4})
